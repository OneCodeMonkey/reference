...
另外一个重要目的，abstrac 类定义好了以后，我们希望继承此抽象类的 子类，一定去实现 父类即抽象类里定义的方法，此时需要用抽象类 abstract 来修饰父类。类似于定义了一个模版类，可以这么理解。
...
3. 抽象类中为什么一定要定义构造方法？
答：强制定义构造方法，其实是为即将继承它的 非抽象子类服务的，因为子类在实例化的时候，调用自己的构造方法之前，会默认先调用父类的构造方法，这样做的目的主要是 给子类中从父类继承过来 的属性 去赋值。所以抽象类中必须要定义构造方法。
